...
export default class GameLogic {
	...
	resetGame = () => {
		// Stats
		(this.playAgainFlag = false), (this.points = 0);
		this.shotsFired = 0;
	...
	writePlayerStats = async () => {
		try {
			...
			// highScore
			const storedHighScore = parseFloat(res[0]?.[1]);
			if (!isNaN(storedHighScore) && storedHighScore > this.points) {
				tempHighScore = storedHighScore;
			} else {
				tempHighScore = this.points;
			}

			// allEnemiesDestroyed
			const storedEnemies = parseFloat(res[1]?.[1]);
			tempAllEnemiesDestroyed = (!isNaN(storedEnemies) ? storedEnemies : 0) + this.enemiesDestroyed;

			// allCoinsCollected
			const storedCoins = parseFloat(res[2]?.[1]);
			tempAllCoinsCollected = (!isNaN(storedCoins) ? storedCoins : 0) + this.coinsCollected;
			...
			console.error('Error writing player stats:', error);
		}
	};
	...
	computeCircle = (circle, position) => {
		...
		let xOfCenter = 0.5 * width + position[0];
		let yOfCenter = 0.5 * height + position[1];
		return {
	...
	checkForCollision = (entities, entity, idOfEntity) => {
		...
		for (let index in entityKeys) {
			...
			if (this.isValidEntity(entities[id])) {
				...
				if (entity.name.includes('Enemy') && entities[id].name === 'Player') {
					let playerCircle = this.computeCircle(entities[id].circle, entities[id].position);
				...
				// Enemy, Projectile collision (square collision)
					...
					if (
						entities[id].position[0] < entity.position[0] + entity.size[0] &&
			...
			if (this.isValidEntity(entities[id]) && id !== 2) {
				if (entities[id].name === 'PlayerProjectile') {
					if (this.isEntityInBounds(entities[id])) {
						entities[id].position = [entities[id].position[0], entities[id].position[1] - 13];
					}
					...
					if (this.isEntityInBounds(entities[id])) {
						entities[id].position = [entities[id].position[0], entities[id].position[1] + 5];
					...
					if (this.isEntityInBounds(entities[id])) {
						entities[id].position = [entities[id].position[0], entities[id].position[1] + 2.5];
					} else {
						this.deleteEntity(id, entities);
						continue;
					}
					...
					if (this.isEntityInBounds(entities[id])) {
						entities[id].position = [entities[id].position[0], entities[id].position[1] + 3];
					} else {
						this.deleteEntity(id, entities);
						continue;
					}
					...
					if (this.isEntityInBounds(entities[id])) {
						entities[id].position = [entities[id].position[0], entities[id].position[1] + 9];
					} else {
						this.deleteEntity(id, entities);
						continue;
					}
	...
	autoFireProjectile = (entities) => {
		if (this.gamePaused || this.hasGameEnded) {
			return entities;
		}

		// Fire every 15 frames (~0.25s at 60fps)
		if (this.framesPlayed % 15 === 0 && this.shotsLeft > 0) {
			this.Sounds.playPlayerProjectile();
			let newProjectile = {
				name: 'PlayerProjectile',
				size: EntitySizes.PROJECTILE_SIZE,
			...
			this.shotsLeft--;
			this.shotsFired++;
		} else if (this.shotsLeft <= 0 && this.framesPlayed % 60 === 0) {
			// Notify out of ammo every second
			this.Sounds.playPlayerOutOfAmmo();
		}
	...
	chanceToSpawnEnemy = (entities) => {
		...
		if (chanceToSpawn < 50 + this.framesPlayed / 900) {
			let id = this.figureId();
			let newEnemy = {};
			let enemyToSpawn = Math.floor(Math.random() * 2);
			if (enemyToSpawn === 0) {
